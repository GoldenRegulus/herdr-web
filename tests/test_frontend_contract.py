...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_desktop_panes_follow_the_full_navigation_frame(self) -> None:
        ...
        self.assertIn('@keyframes working-status-shimmer', stylesheet)
        self.assertIn('.sheet-item[data-status="working"]::after', stylesheet)
        self.assertIn('animation: working-status-shimmer 2.6s ease-in-out infinite;', stylesheet)
        self.assertIn("color-mix(in srgb, var(--theme-green) 10%, transparent)", stylesheet)
        self.assertIn("color-mix(in srgb, var(--theme-overlay-1) 9%, transparent)", stylesheet)
        ...
        self.assertEqual(stylesheet.count('border-right: 3px solid transparent;'), 1)
        self.assertIn('.sheet-item[data-status="working"]', stylesheet)
        self.assertIn('.sheet-item[data-status="done"] {', stylesheet)
        self.assertNotIn('.sheet-item[data-status="done"] { border-right-color', stylesheet)
        self.assertNotIn('.sheet-item[data-status="blocked"] { border-right-color', stylesheet)
    ...
    def test_mobile_navigation_uses_structured_backend_api(self) -> None:
        ...
        self.assertIn("backendApiUrl(backend, 'focus')", application)
        self.assertIn("function markTabSeen(tabId)", application)
        self.assertNotIn("if (!mobileQuery.matches || !tabId)", application)
        self.assertIn("requestNavigationFocus('tab', tabId)", application)
        self.assertIn("if (navigationFocusRequest) return navigationFocusRequest", application)
        self.assertIn("markTabSeen(pane.tab_id)", application)
        self.assertIn("markTabSeen(tab.tab_id)", application)
    ...
    def test_mobile_sheet_uses_an_expandable_flat_animated_tree(self) -> None:
        ...
        self.assertIn("color-mix(in srgb, var(--theme-accent) 12%, var(--theme-surface-dim))", stylesheet)
    ...
    def test_mobile_uses_panes_with_touch_scrolling(self) -> None:
        ...
        navigation_row = document[document.index('id="mobile-navigation-row"'):]
        self.assertLess(default_row.index('id="mobile-arrows"'), default_row.index('id="mobile-keyboard-lock"'))
        self.assertNotIn('id="mobile-snapshot"', document)
        self.assertIn('id="mobile-mouse-mode"', navigation_row[:navigation_row.index("</nav>")])
        self.assertNotIn('id="mobile-mouse-mode"', default_row)
        self.assertIn('path d="M6 12h.01M12 12h.01M18 12h.01"', document)
        self.assertIn('#mobile-navigation-mode .mobile-bar-icon { stroke-width: 3; }', stylesheet)
        self.assertIn("function syncAutoSnapshotSelection()", application)
        self.assertIn("openTerminalSnapshot(record, true, { x: touchPointerX, y: touchPointerY });", application)
        self.assertIn("function selectSnapshotWordAt(pane, x, y, attempt = 0)", application)
        self.assertIn("Fall back to the nearest", application)
        self.assertIn("selectSnapshotWordAt(pane, point.x, point.y);", application)
        self.assertIn("snapshot.closeTimer = setTimeout(() => {", application)
        self.assertIn("const mobileModifierMode = { control: 'off', alt: 'off', shift: 'off' };", application)
        self.assertIn("const next = { off: 'once', once: 'hold', hold: 'off' };", application)
        self.assertIn("function consumeMobileModifiers()", application)
        self.assertIn("consumeMobileModifiers();", application)
        self.assertIn('#mobile-modifiers button[data-mode="once"]', stylesheet)
        ...
        self.assertIn("function openTerminalSnapshot(pane, auto = false, point = undefined)", application)
        self.assertIn("function closeTerminalSnapshot(pane, syncControls = true)", application)
        self.assertNotIn("mobileSnapshotButton", application)
        self.assertNotIn("renderMobileSnapshotButton", application)
        ...
        self.assertIn("auto, openedAt: performance.now(), selected: false, closeTimer: undefined,", application)
    ...
    def test_mobile_input_shadows_the_visible_line_and_leaves_editability_to_the_shell(self) -> None:
        ...
        composition_start = application[
            ...
            :application.index("function handleMobilePredictionCompositionUpdate(event)")
        ]
        composition_end = application[
            application.index("function handleMobilePredictionCompositionEnd(event)")
            :application.index("function noteMobilePredictionTerminalData(pane, data)")
        ]
        self.assertIn("pane.mobilePredictionComposition = true;", composition_start)
        self.assertNotIn("restoreMobilePredictionHelper", composition_start)
        self.assertNotIn("clearMobileBackspaceSentinel", application)
        self.assertIn("applyMobileTextValue(pane, helper.value, mobileHelperCaret(helper), true, true);", composition_end)
        self.assertIn("const androidKeyboard = /Android/.test(navigator.userAgent)", application)
        self.assertIn("if (!helper || !nativeKeyboardInput || !mobileQuery.matches) return;", application)
        self.assertIn("if (!nativeKeyboardInput || !mobileQuery.matches || mobileKeyboardLocked) return;", application)
        self.assertIn("paneTerminalKeyHandler(record, event)", application)
    ...
    def test_mobile_caret_moves_the_terminal_through_ordered_input(self) -> None:
        ...
        self.assertIn("|| pane.mobilePredictionComposition", application)
    ...
    def test_mobile_backspace_uses_native_text_edits_with_an_empty_helper_fallback(self) -> None:
        ...
        self.assertNotIn("document.execCommand('insertText'", application)
        self.assertIn("event.inputType !== 'deleteContentBackward'", application)
        self.assertIn("The native textarea owns Backspace and its repeat", application)
        self.assertIn("if (!iosKeyboard || !mobileQuery.matches || mobileKeyboardLocked", application)
        self.assertIn("|| (imeKey && Boolean(pane?.mobilePredictionComposition))", application)
